...
use aptos_move_vm_runtime::{Module, ModuleStorage, RuntimeEnvironment, Script, WithRuntimeEnvironment, NoOpLayoutCache};
...
impl TDelayedFieldView for AptosCustomState {
    ...
    ) -> Result<BTreeMap<StateKey, (StateValueMetadata, u64, triomphe::Arc<MoveTypeLayout>)>, PanicError> {
        Err(PanicError::CodeInvariantError("unreachable".to_string()))
    }
...
impl NoOpLayoutCache for AptosCustomState {}
...
impl AptosCustomState {
    ...
    pub fn default_env() -> aptos_vm_environment::environment::AptosEnvironment {
        ...
        let view = crate::custom_state_view::CustomStateView::new(&tmp);
